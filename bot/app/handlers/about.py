...
async def about(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "about":
        ...
        about_text = f"🎮 DotaVision Bot\n\n" \
                     ...
                     f"🤖 It operates using AI models, predicting matches outcomes based on heroes performances and stats in pro and high ranked immortal games. Right now bot predicts based ONLY on heroes, no accounting players and teams (which fill be added in the future). AI is constantly improving and will get better over time\n\n" \
                     ...
                     f"🤖 Бот работает с помощью AI-моделей, предсказывая исходы матчей на основе статистики по героям, динамике в топ иммортал и про играх. Сейчас бот делает предикты учитывая ТОЛЬКО героев и пики, не беря в счет команды и игроков (это будет добавлено в будущем). AI находится в стадии активного развития и постоянно переобучается на новых данных\n\n" \
